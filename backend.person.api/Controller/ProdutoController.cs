...
namespace backend.person.api.Controller
{
    ...
    public class ProdutoController : ControllerBase
    {
        ...
        public IActionResult GetList([FromQuery] string? ids = null, string? descricao = null,
         int page = 0, int pageSize = 0, int? idMarca = null, int? idCateggoria = null , int? idMaterial = null )
        {
            try
            {

                return Ok(_produtoService.GetList(ids,descricao,page,pageSize,idMarca,idCateggoria,idMaterial ));
            }
            catch (Exception e)
            {
                return BadRequest(e.Message);
            ...
            int page = 0, int pageSize = 0, int? idMarca = null, int? idCategoria = null , int? idMaterial = null)
        {
            try
            {
                ...
                return Ok(_produtoService.GetVw(ids, descricao, page, pageSize, idMarca, idCategoria, idMaterial));
            }
            catch (Exception e)
            {
                return BadRequest(e.Message);
